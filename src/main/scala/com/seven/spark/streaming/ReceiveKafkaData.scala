  ...
  def main(args: Array[String]): Unit = {
    ...
    kafkaStream.map(_.value()).foreachRDD(rdd => {
      if (!rdd.isEmpty()) rdd.foreachPartition(x => {
        ...
        x.foreach(row => {
          val maxTime = "2100-01-01 00:00:00.0"
          ...
          val money = line(7).toDouble //金额

          val account = line(10) //支付账号

          val netId = line(15) //网点id

          val pointId = line(16) //点位id

          val operateId = line(13) //运营商id

          val officeId = line(14) //办事处id
          ...
          //          val machine = orderId.substring(3, 11) //去除YST后的机器id

          val num = orderId.length

          val orderTime = orderId.substring(num - 16, num - 4)

          val hashNumber = orderId.substring(num - 4, num)

          val invertedTime = (df.parse(maxTime).getTime - sdf.parse(orderTime).getTime)/1000 //倒排时间

          val rowKey = Bytes.toBytes(hashNumber + invertedTime + machineId.hashCode) //拼接rowkey  随机码+时间倒排+机器id的hash值

//          val rowKey = Bytes.add(Bytes.toBytes(hashNumber),Bytes.toBytes(invertedTime),Bytes.toBytes(machineId.hashCode))
          ...
          put.addColumn(Bytes.toBytes(family), Bytes.toBytes("OPERATEID"), Bytes.toBytes(operateId)) //插入运营商
          put.addColumn(Bytes.toBytes(family), Bytes.toBytes("OFFICEID"), Bytes.toBytes(officeId)) //插入办事处
